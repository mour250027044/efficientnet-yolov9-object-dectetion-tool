...
from .modules.hybrid_blocks import SPDADown, FABlock, Adapter, MyHGBlock
from .modules.efficientnet_backbone import EfficientNetBackbone
from .modules.custom_layers import CBAM, ASPP, BiFPN, DropBlock
...
def parse_model(d, ch, verbose=True):
    ...
    """
    import ast
    # --- ADDED: Need access to custom modules for 'is' check ---
    from ultralytics.nn.modules.efficientnet_backbone import EfficientNetBackbone
    from ultralytics.nn.modules.hybrid_blocks import Adapter, FABlock, MyHGBlock, SPDADown
    # --- END ADDITION ---
    ...
    if act:
        # Get activation function from torch.nn directly
        activation_cls = getattr(torch.nn, act) if hasattr(torch.nn, act) else torch.nn.SiLU
        Conv.default_act = activation_cls() # Instantiate the activation
        if verbose:
    ...
    known_modules = {**globals(), **torch.nn.__dict__} 

    for i, (f, n, m_str, args) in enumerate(d["backbone"] + d["head"]):
        # --- Find module class ---
        if m_str.startswith("nn."):
            m = getattr(torch.nn, m_str[3:], None)
        elif m_str.startswith("torchvision.ops."):
            try:
                m = getattr(__import__("torchvision").ops, m_str[16:])
            except (ImportError, AttributeError):
                m = None 
        else:
            m = known_modules.get(m_str)

        
        if m is None:
            raise NameError(f"Module '{m_str}' not found. Check spelling or ensure it's imported/defined correctly.")
        
        for j, a in enumerate(args):
            if isinstance(a, str):
                with contextlib.suppress(ValueError, NameError): # Added NameError catch
                    args[j] = locals().get(a, None) if a in locals() else ast.literal_eval(a)


        n = n_ = max(round(n * depth), 1) if n > 1 else n

        
        if m is EfficientNetBackbone:
            c1 = ch[f]
            backbone_name = args[0]
            if 'b0' in backbone_name: c2 = 1280
            elif 'b1' in backbone_name: c2 = 1280
            elif 'b2' in backbone_name: c2 = 1408
            elif 'b3' in backbone_name: c2 = 1536 # Correct for B3
            elif 'b4' in backbone_name: c2 = 1792
            elif 'b5' in backbone_name: c2 = 2048
            elif 'b6' in backbone_name: c2 = 2304
            elif 'b7' in backbone_name: c2 = 2560
            else: raise ValueError(f"Unknown EfficientNet scale: {backbone_name}")
            ...
            c1, c2_arg = ch[f], args[0]
            c2 = make_divisible(min(c2_arg, max_channels) * width, 8) if c2_arg != nc else nc            
            current_args = [c1, c2, *args[1:]]
            if m in repeat_modules:
                current_args.insert(2, n)
                n = 1
        elif m is Concat:
            c1 = [ch[x] for x in f] 
            c2 = sum(c1)
            ...
            c1 = ch[f]
            c2 = c1
            current_args = [c1] + args if isinstance(m, torch.nn.Module) else args 
            

        if m in {EfficientNetBackbone, MyHGBlock, SPDADown, FABlock, Adapter, DropBlock, CBAM}:
            m_ = torch.nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)
        else:
            m_ = torch.nn.Sequential(*(m(*current_args) for _ in range(n))) if n > 1 else m(*current_args)
